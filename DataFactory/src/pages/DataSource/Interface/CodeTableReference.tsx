...
interface CodeTableReferenceProps {
  ...
  onSelect: (id: number) => void; // 新增回调函数
}

const CodeTableReference: React.FC<CodeTableReferenceProps> = ({ visible, data, onCancel,onSelect }) => {
  ...
  const handleOk = () => {
    ...
    if (selectedTableId) {
      onSelect(selectedTableId); // 调用回调函数传递选中的 id
    }
    onCancel();
  };
...
export default CodeTableReference;